  ...
  namespace IO
  {
    ...
    template <typename T>
      struct BinaryExtractor
      {
        ...
        BinaryExtractor( MaskPolicy<T> policy = MaskPolicy<T>() ) : counter(0), policy(policy)
        {
      ...
      class BinaryReader : public Reader
    {
      public:
        ...
        virtual bool extract( std::vector< std::pair< double, boost::shared_ptr<T> > >& entries, MaskPolicy<T> policy = MaskPolicy<T>() )
        {
          // How many elements?
          ...
          entries.assign(extractor.elements.begin(), extractor.elements.end());
